...
struct kchecksec_t {
  ...
  // == Surfaces ==
  int unpriv_userfaultfd;   // /proc/sys/vm/unprivileged_userfaultfd (or -1 if
                            // N/A)
  char *dev_userfaultfd_ls; // `ls -l /dev/userfaultfd` 結果 or "none"
  int unpriv_bpf_disabled;  // /proc/sys/kernel/unprivileged_bpf_disabled (or -1
                            // if N/A)
  ...
  int unpriv_userns_clone;  // sysctl kernel.unprivileged_userns_clone (or -1 if
                            // N/A)
