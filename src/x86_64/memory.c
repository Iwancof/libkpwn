...
void *pti_mmap(logf_t log, unsigned long pgd, unsigned long pud,
  ...
  log("pti_mmap(%p, %#lx, %#x, %#x, %d, %#lx) = %p", dest, len, prot, flags,
      fildes, off, ret);

  return ret;
}

size_t phy_to_pte(size_t phys) { return phys | 0x67ULL | (1ULL << 63); }
size_t pte_to_phy(size_t pte) { return pte & ~(1ULL << 63) & ~0xFFFULL; }
