...
void hexdump(logf_t log, void* content, size_t len) {
  uint8_t* ptr = content;
  static char val_buf[MAX_BUFFER_SIZE],
      char_buf[MAX_BUFFER_SIZE];

  log("-- hexdump of %p, len = %zu --", content, len);
...
#define append(buf, ptr, ...)                       \
  ptr += snprintf(ptr, buf + MAX_BUFFER_SIZE - ptr, \
                  __VA_ARGS__);

    append(val_buf, cur_vp, "%*c",
           (int)(1 + 3 * padding_len), ' ');
    ...
    for (ssize_t i = draw_len - 1; 0 <= i; i--) {
      ...
      append(val_buf, cur_vp, "%s%02x" COLOR_RESET " ", col,
             v);
      append(char_buf, cur_cp, "%s%c" COLOR_RESET, col,
             chr);
    }
